...
from .forms import UpdateCreatePostForm , CommentCreateForm ,CommentReplyForm , PostSearchForm
...
class PostDetailView(View):
    ...
    def get(self, request, *args, **kwargs):
        comments = self.post_inctance.pcomments.filter(is_reply=False)
        can_like = False
        if request.user.is_authenticated and self.post_inctance.user_can_like(request.user):
            can_like = True

        return render(request,"home/detail.html", {'post':self.post_inctance,'comments':comments,
...
class AddReplyView(LoginRequiredMixin,View):
    ...
    def post(self, request, post_id,comment_id):
        ...
        if form.is_valid():
            ...
            reply.is_reply = True
            reply.save()
            messages.success(request, 'Your reply has been posted','success')
        return redirect('home:post_detail', post.id,post.slug)
